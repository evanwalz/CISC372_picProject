    ...
    for (int row = 0; row < height; row++){
        for (int col = 0; col < width; col++){
            for (int bit = 0; bit < bpp; bit++){
                destImage->data[Index(col,row,width,bit,bpp)] =
...
int main(int argc, char** argv){
    ...
    size_t bytes = (size_t)destImage.width * (size_t)destImage.height * (size_t)destImage.bpp;
    destImage.data = (uint8_t*)malloc(bytes);
    if (!destImage.data){
        fprintf(stderr, "Allocation failed for destImage (%zu bytes)\n", bytes);
